...
public enum CucumberTestConfiguration {
    ...
    public static func fromEnvironment() -> CucumberTestConfiguration {
        ...
        if let envMode = ProcessInfo.processInfo.environment["CUCUMBER_TEST_MODE"] {
            ...
            case "step", "steps":
                return .stepBased
            default:
                print("Warning: Invalid CUCUMBER_TEST_MODE '\(envMode)'. Valid values: 'step', 'scenario'. Using default 'step'.")
                return .stepBased
            }
        }
        ...
        if let testBundle = (Cucumber.shared as? StepImplementation)?.bundle,
           let plistMode = testBundle.infoDictionary?["CucumberTestMode"] as? String {
            switch plistMode.lowercased() {
            case "scenario", "scenarios":
                return .scenarioBased
            case "step", "steps":
                return .stepBased
            default:
                print("Warning: Invalid CucumberTestMode '\(plistMode)' in Info.plist. Valid values: 'step', 'scenario'. Using default 'step'.")
                return .stepBased
            }
        }
